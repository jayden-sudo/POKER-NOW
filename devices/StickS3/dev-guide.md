...
理由:三項全屬本機 UI/板級行為,零協定影響;放系統選單(UI_MENU 全畫面可達;
本板為長按 OK)不增加任何新手勢,不破壞 2 鍵心智。實作位置:選單邏輯在共用 `app_flow.c`
...
- 系統選單按裁定 R9 擴充:首行 `BATT <pct>%`(0xFF→`BATT USB`,充電中加 `CHG`,
  ...
  `[VOLUME]` 照產品 §5(10/40/70/100 + 試播 V_BEEP)。兩者選檔邏輯共用同一個
  cycle-item 實作,只差回呼。
...
（腳本以 `cc -std=c11 -Wall -Wextra -Werror -DPK_HOST_TEST` 編譯 `test_hand_eval.c` +
`hand_eval.c` + `side_pot.c`,路徑相對腳本自身解析,任何工作目錄皆可執行。）
