/* hal_audio.c -- ES8311 + AW8737 + I2S(指南 §15)。
 * 輸出流常開靜音填充;play_at 樣本級切入;電池供電音量鉗 ≤75%。 */
...
/* 耳測信標(§21.R 驗收用):開機就緒後每 5 秒播一次 V_BEEP,共 3 次。 */
...
esp_err_t hal_audio_init(void)
{
    ...
    /* AW8737 功放常開:PM1 GPIO3 推挽恆高(§21.R;讀回自證見 m5pm1 日誌) */
    esp_err_t amp = m5pm1_amp_set(1);
    if (amp != ESP_OK) ESP_LOGE(TAG, "AW8737 amp enable failed (%s)", esp_err_to_name(amp));
